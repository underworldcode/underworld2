...
Fn::Map::func Fn::Map::getFunction( IOsptr sample_input )
{
    ...
    if (keyfuncout->size() != 1)
        throw std::invalid_argument( "Key function appears to return a non-scalar value." );
    ...
    if (_defaultFunc)
    {
        ...
        if (!_defaultFuncOut)
            throw std::invalid_argument( "Default function does not appear to return a 'double' value, as required." );
    ...
    for (unsigned ii=0; ii<_isIndexInMap.size(); ii++ )
    {
        if (_isIndexInMap[ii])
        {
            ...
            {
                std::stringstream ss;
                ss << "Function with key " << ii << " does not appear to return a 'double' value, as required";
