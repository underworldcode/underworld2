...
/** Default extra factor to malloc for particles array. This is because the
number of particles on each process may increase due to advection,
splitting/merging etc, and we want to avoid reallocing if possible. */
...
Swarm *Swarm_New(Name name, AbstractContext *context, void *cellLayout,
                 void *particleLayout, Dimension_Index dim, SizeT particleSize,
                 ExtensionManager_Register *extensionMgr_Register,
                 Variable_Register *variable_Register, MPI_Comm comm,
                 void *ics) {
  Swarm *self = _Swarm_DefaultNew(name);

  _Swarm_Init(self, context, cellLayout, particleLayout, dim, particleSize,
              DEFAULT_CELL_PARTICLE_TBL_DELTA, DEFAULT_EXTRA_PARTICLES_FACTOR,
              extensionMgr_Register, variable_Register, False, comm, ics);
...
void _Swarm_Init(Swarm *self, AbstractContext *context, void *cellLayout,
                 void *particleLayout, Dimension_Index dim, SizeT particleSize,
                 Particle_InCellIndex cellParticleTblDelta,
                 double extraParticlesFactor,
                 ExtensionManager_Register *extensionMgr_Register,
                 Variable_Register *variable_Register, Bool createGlobalIdVar,
                 MPI_Comm comm, void *ics) {
  ...
  /* Check point and reload by default - only things like integration swarms
   * will turn this off */
  ...
  /* Check that if either the CellLayout or ParticleLayout is inappropriate to
     be checkpointed, we set tthe appropriate flags -- PatrickSunter, 22 August
     2006 */
  ...
  Journal_Firewall(extraParticlesFactor > 0.0, errorStream,
                   "Error - in %s: extraParticlesFactor "
                   "given as %.3f, but this must be greater than zero to allow "
                   "swarm to be realloc'ed larger "
                   "if necessary\n",
                   extraParticlesFactor);

  self->swarmVariable_Register = SwarmVariable_Register_New(variable_Register);

  self->particleExtensionMgr =
      ExtensionManager_New_OfStruct("particle", self->particleSize);
  ExtensionManager_Register_Add(extensionMgr_Register,
                                self->particleExtensionMgr);

  self->owningCellVariable = Swarm_NewScalarVariable(
      self, "OwningCell", GetOffsetOfMember(particle, owningCell),
      StgVariable_DataType_Int); /* Should be unsigned int */

  lcReg = LiveComponentRegister_GetLiveComponentRegister(); /* only needed for
                                                               tests like
                                                               GaussLayoutSingleCellSuite
                                                               which don't have
                                                               liveComponent_Registers
                                                             */
  if (lcReg) {
    LiveComponentRegister_Add(LiveComponentRegister_GetLiveComponentRegister(),
                              (Stg_Component *)self->owningCellVariable);
    LiveComponentRegister_Add(
        LiveComponentRegister_GetLiveComponentRegister(),
        (Stg_Component *)self->owningCellVariable->variable);
  }
  ...
  if (createGlobalIdVar && self->gidExtHandle == (unsigned)-1) {
    ...
    self->gidExtHandle = ExtensionManager_Add(self->particleExtensionMgr,
                                              (Name)self->type, sizeof(int));

    // create a swarm variable for the globalId extension
    gidExt = ExtensionManager_Get(self->particleExtensionMgr, &particle,
                                  self->gidExtHandle);
    self->globalIdVariable = Swarm_NewScalarVariable(
        self, "GlobalId", (ArithPointer)gidExt - (ArithPointer)&particle,
        StgVariable_DataType_Int);

    if (lcReg) {
      LiveComponentRegister_Add(
          LiveComponentRegister_GetLiveComponentRegister(),
          (Stg_Component *)self->globalIdVariable);
      LiveComponentRegister_Add(
          LiveComponentRegister_GetLiveComponentRegister(),
          (Stg_Component *)self->globalIdVariable->variable);
    }
  }

  self->swarmReg_I =
      Swarm_Register_Add(Swarm_Register_GetSwarm_Register(), self);
...
void *_Swarm_ParticleInCellAt(void *swarm, Cell_Index cell_I,
...
void _Swarm_Delete(void *swarm) {
  ...
  if (Swarm_Register_Get(Swarm_Register_GetSwarm_Register(), self->name))
    Swarm_Register_RemoveIndex(Swarm_Register_GetSwarm_Register(),
                               self->swarmReg_I);
...
void _Swarm_Print(void *swarm, Stream *stream) {
  ...
  Journal_Printf(swarmStream,
                 "\tcellPointCountTbl[0-%u]: ", self->cellDomainCount);
  ...
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
    ...
    Journal_Printf(swarmStream, "\tcellPointTbl[%u][0-%u]: ", cell_I,
                   self->cellPointCountTbl[cell_I]);
  ...
  Journal_Printf(swarmStream,
                 "\tcellParticleCountTbl[0-%u]: ", self->cellDomainCount);
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
  ...
  Journal_Printf(swarmStream,
                 "\tcellParticleSizeTbl[0-%u]: ", self->cellDomainCount);
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
  ...
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
    ...
    Journal_Printf(swarmStream, "\t\tcellParticleTbl[%u][0-%u]:\n", cell_I,
                   self->cellParticleCountTbl[cell_I]);
    for (cParticle_I = 0; cParticle_I < self->cellParticleCountTbl[cell_I];
         cParticle_I++) {
...
void *_Swarm_Copy(void *swarm, void *dest, Bool deep, Name nameExt,
                  PtrMap *ptrMap) {
  Swarm *self = (Swarm *)swarm;
  Swarm *newSwarm;
  PtrMap *map = ptrMap;
  ...
  if (deep) {
    /* Classes */
    newSwarm->cellLayout = (CellLayout *)Stg_Class_Copy(self->cellLayout, NULL,
                                                        deep, nameExt, map);
    newSwarm->particleLayout = (ParticleLayout *)Stg_Class_Copy(
        self->particleLayout, NULL, deep, nameExt, map);
    newSwarm->debug =
        (Stream *)Stg_Class_Copy(self->debug, NULL, deep, nameExt, map);
    newSwarm->particleExtensionMgr = (ExtensionManager *)Stg_Class_Copy(
        self->particleExtensionMgr, NULL, deep, nameExt, map);
    newSwarm->commHandlerList = (Stg_ObjectList *)Stg_Class_Copy(
        self->commHandlerList, NULL, deep, nameExt, map);

    /* Arrays */
    if ((newSwarm->cellPointCountTbl =
             PtrMap_Find(map, self->cellPointCountTbl)) == NULL) {
      if (self->cellPointCountTbl) {
        newSwarm->cellPointCountTbl =
            Memory_Alloc_Array(Cell_PointIndex, newSwarm->cellDomainCount,
                               "Swarm->cellPointCountTbl");
        memcpy(newSwarm->cellPointCountTbl, self->cellPointCountTbl,
               newSwarm->cellDomainCount * sizeof(Cell_PointIndex));
        PtrMap_Append(map, self->cellPointCountTbl,
                      newSwarm->cellPointCountTbl);
      } else {
    ...
    if ((newSwarm->cellPointTbl = PtrMap_Find(map, self->cellPointTbl)) ==
        NULL) {
      if (newSwarm->cellPointCountTbl && self->cellPointTbl) {
        Index cell_I;

        newSwarm->cellPointTbl = Memory_Alloc_2DComplex(
            Cell_Point, newSwarm->cellDomainCount, newSwarm->cellPointCountTbl,
            "Swarm->cellPointTbl");
        for (cell_I = 0; cell_I < newSwarm->cellDomainCount; cell_I++) {
          memcpy(newSwarm->cellPointTbl[cell_I], self->cellPointTbl[cell_I],
                 newSwarm->cellPointCountTbl[cell_I] * sizeof(Cell_Point));
        }
    ...
    if ((newSwarm->cellParticleCountTbl =
             PtrMap_Find(map, self->cellParticleCountTbl)) == NULL) {
      if (self->cellParticleCountTbl) {
        newSwarm->cellParticleCountTbl =
            Memory_Alloc_Array(Particle_InCellIndex, newSwarm->cellDomainCount,
                               "Swarm->cellParticleCountTbl");
        memcpy(newSwarm->cellParticleCountTbl, self->cellParticleCountTbl,
               newSwarm->cellDomainCount * sizeof(Particle_InCellIndex));
        PtrMap_Append(map, self->cellParticleCountTbl,
                      newSwarm->cellParticleCountTbl);
      } else {
    ...
    if ((newSwarm->cellParticleSizeTbl =
             PtrMap_Find(map, self->cellParticleSizeTbl)) == NULL) {
      if (self->cellParticleSizeTbl) {
        newSwarm->cellParticleSizeTbl =
            Memory_Alloc_Array(Particle_InCellIndex, newSwarm->cellDomainCount,
                               "Swarm->cellParticleSizeTbl");
        memcpy(newSwarm->cellParticleSizeTbl, self->cellParticleSizeTbl,
               newSwarm->cellDomainCount * sizeof(Particle_InCellIndex));
        PtrMap_Append(map, self->cellParticleSizeTbl,
                      newSwarm->cellParticleSizeTbl);
      } else {
    ...
    if ((newSwarm->cellParticleTbl = PtrMap_Find(map, self->cellParticleTbl)) ==
        NULL) {
      if (newSwarm->cellParticleCountTbl && self->cellParticleTbl) {
        Index cell_I;

        newSwarm->cellParticleTbl =
            Memory_Alloc_Array(Cell_Particles, newSwarm->cellDomainCount,
                               "Swarm->cellParticleTbl");
        for (cell_I = 0; cell_I < newSwarm->cellDomainCount; cell_I++) {
          if (newSwarm->cellParticleCountTbl[cell_I]) {
            newSwarm->cellParticleTbl[cell_I] = Memory_Alloc_Array(
                Particle_Index, newSwarm->cellParticleCountTbl[cell_I],
                "Swarm->cellParticleTbl[]");
            memcpy(newSwarm->cellParticleTbl[cell_I],
                   self->cellParticleTbl[cell_I],
                   newSwarm->cellParticleCountTbl[cell_I] *
                       sizeof(Particle_Index));
          } else {
    ...
    if ((newSwarm->particles = PtrMap_Find(map, self->particles)) == NULL) {
      if (self->particles) {
        newSwarm->particles = (Particle_List)ExtensionManager_Malloc(
            newSwarm->particleExtensionMgr, newSwarm->particlesArraySize);
        memcpy(
            newSwarm->particles, self->particles,
            newSwarm->particlesArraySize *
                ExtensionManager_GetFinalSize(newSwarm->particleExtensionMgr));
    ...
    if (self->shadowTablesBuilt) {
      ...
      if ((newSwarm->shadowCellParticleCountTbl =
               PtrMap_Find(map, self->shadowCellParticleCountTbl)) == NULL) {
        if (self->shadowCellParticleCountTbl) {
          newSwarm->shadowCellParticleCountTbl = Memory_Alloc_Array(
              Particle_InCellIndex, newSwarm->cellDomainCount,
              "Swarm->shadowCellParticleCountTbl");
          memcpy(newSwarm->shadowCellParticleCountTbl,
                 self->shadowCellParticleCountTbl,
                 newSwarm->cellDomainCount * sizeof(Particle_InCellIndex));
          PtrMap_Append(map, self->shadowCellParticleCountTbl,
                        newSwarm->shadowCellParticleCountTbl);
        } else {
          newSwarm->shadowCellParticleCountTbl = NULL;
        }
      }

      if ((newSwarm->shadowCellParticleTbl =
               PtrMap_Find(map, self->shadowCellParticleTbl)) == NULL) {
        if (newSwarm->shadowCellParticleCountTbl &&
            self->shadowCellParticleTbl) {
          Index cell_I;

          newSwarm->shadowCellParticleTbl =
              Memory_Alloc_Array(Cell_Particles, newSwarm->cellDomainCount,
                                 "Swarm->shadowCellParticleTbl");
          for (cell_I = 0; cell_I < newSwarm->cellDomainCount; cell_I++) {
            if (newSwarm->shadowCellParticleCountTbl[cell_I]) {
              newSwarm->shadowCellParticleTbl[cell_I] = Memory_Alloc_Array(
                  Particle_Index, newSwarm->shadowCellParticleCountTbl[cell_I],
                  "Swarm->shadowCellParticleTbl[]");
              memcpy(newSwarm->shadowCellParticleTbl[cell_I],
                     self->shadowCellParticleTbl[cell_I],
                     newSwarm->shadowCellParticleCountTbl[cell_I] *
                         sizeof(Particle_Index));
            } else {
              newSwarm->shadowCellParticleTbl[cell_I] = NULL;
            }
          }
          PtrMap_Append(map, self->shadowCellParticleTbl,
                        newSwarm->shadowCellParticleTbl);
        } else {
      ...
      if ((newSwarm->shadowParticles =
               PtrMap_Find(map, self->shadowParticles)) == NULL) {
        if (self->shadowParticles) {
          newSwarm->shadowParticles = (Particle_List)ExtensionManager_Malloc(
              newSwarm->particleExtensionMgr, newSwarm->shadowParticleCount);
          memcpy(newSwarm->shadowParticles, self->shadowParticles,
                 newSwarm->shadowParticleCount *
                     ExtensionManager_GetFinalSize(
                         newSwarm->particleExtensionMgr));
    ...
    newSwarm->ics = self->ics ? (VariableCondition *)Stg_Class_Copy(
                                    self->ics, NULL, deep, nameExt, map)
                              : NULL;
  } else {
...
void *_Swarm_DefaultNew(Name name) {
   ...
   * current _New function or another parent _New function further up the
   * hierachy */
...
void _Swarm_AssignFromXML(void *swarm, Stg_ComponentFactory *cf, void *data) {
  ...
  context = Stg_ComponentFactory_ConstructByKey(
      cf, self->name, (Dictionary_Entry_Key) "Context", AbstractContext, False,
      data);
  if (!context)
    context = Stg_ComponentFactory_ConstructByName(
        cf, (Name) "context", AbstractContext, False, data);
  ...
  dim = Stg_ComponentFactory_GetRootDictUnsignedInt(
      cf, (Dictionary_Entry_Key) "dim", 0);
  dim = Stg_ComponentFactory_GetUnsignedInt(cf, self->name,
                                            (Dictionary_Entry_Key) "dim", dim);

  cellLayout = Stg_ComponentFactory_ConstructByKey(
      cf, self->name, (Dictionary_Entry_Key)CellLayout_Type, CellLayout, True,
      data);
  particleLayout = Stg_ComponentFactory_ConstructByKey(
      cf, self->name, (Dictionary_Entry_Key)ParticleLayout_Type, ParticleLayout,
      False, data);
  ...
  cellParticleTblDelta = Stg_ComponentFactory_GetUnsignedInt(
      cf, self->name, (Dictionary_Entry_Key) "cellParticleTblDelta",
      DEFAULT_CELL_PARTICLE_TBL_DELTA);
  extraParticlesFactor = Stg_ComponentFactory_GetDouble(
      cf, self->name, (Dictionary_Entry_Key) "extraParticlesFactor",
...

  {
    ...
    components = (Stg_Component **)Stg_ComponentFactory_ConstructByList(
        cf, self->name, (Dictionary_Entry_Key) "ParticleCommHandlers",
        Stg_ComponentFactory_Unlimited, ParticleCommHandler, False, &count,
        data);

    if (count == 0) {
      Journal_Printf(self->debug,
                     "Warning: Swarm has 0 Communication handlers..!\n");
    } else {
      for (i = 0; i < count; i++) {
  ...
  ic = Stg_ComponentFactory_ConstructByKey(cf, self->name,
                                           (Dictionary_Entry_Key) "IC",
                                           VariableCondition, False, data);

  createGlobalIdVar = Stg_ComponentFactory_GetBool(
      cf, self->name, (Dictionary_Entry_Key) "createGlobalId", False);

  _Swarm_Init(self, context, cellLayout, particleLayout, dim,
              sizeof(IntegrationPoint), cellParticleTblDelta,
              extraParticlesFactor, extensionManagerRegister, variable_Register,
              createGlobalIdVar, comm, ic);
}
  ...
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
    ...
    if (self->shadowTablesBuilt) {
      if (self->shadowCellParticleTbl[cell_I]) {
        ...
        self->shadowCellParticleTbl[cell_I] = NULL;
      }
    }
  }
...
void _Swarm_BuildCells(void *swarm, void *data) {
  ...
  Journal_DPrintf(
      self->debug,
      "CellLayout \"%s\" (of type %s) returned cell counts of:\n"
      "cellLocalCount: %u, cellShadowCount: %u, cellDomainCount: %u",
      self->cellLayout->name, self->cellLayout->type, self->cellLocalCount,
      self->cellShadowCount, self->cellDomainCount);

  self->cellPointCountTbl = Memory_Alloc_Array(
      Cell_PointIndex, self->cellDomainCount, "Swarm->cellPointCountTbl");
  pointCount = 0;

  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
    self->cellPointCountTbl[cell_I] =
        CellLayout_PointCount(self->cellLayout, cell_I);
    pointCount += self->cellPointCountTbl[cell_I];
  }

  self->cellPointTbl =
      Memory_Alloc_2DComplex(Cell_Point, self->cellDomainCount,
                             self->cellPointCountTbl, "Swarm->cellPointTbl");

  self->cellParticleCountTbl =
      Memory_Alloc_Array(Particle_InCellIndex, self->cellDomainCount,
                         "Swarm->cellParticleCountTbl");
  self->cellParticleSizeTbl =
      Memory_Alloc_Array(Particle_InCellIndex, self->cellDomainCount,
                         "Swarm->cellParticleSizeTbl");
  self->cellParticleTbl = Memory_Alloc_Array(
      Cell_Particles, self->cellDomainCount, "Swarm->cellParticleTbl");
...
void _Swarm_BuildParticles(void *swarm, void *data) {
  Swarm *self = (Swarm *)swarm;
  unsigned v_i;
  /* Need to do this first - as the particleLayout may be dependent on a mesh
   * etc */
  Stg_Component_Build(self->particleLayout, data, False);
  ...
  /* We allocate extra space to try & avoid avoid reallocing later as particles
   * are advected etc. */
  self->particlesArrayDelta =
      (Particle_Index)((double)self->particleLocalCount *
                       self->extraParticlesFactor);
  ...
  self->particlesArraySize =
      self->particleLocalCount + self->particlesArrayDelta;

  self->particles = (Particle_List)ExtensionManager_Malloc(
      self->particleExtensionMgr, self->particlesArraySize);
  /*
  ...
  */
  ...
  for (cell_I = 0; cell_I < self->cellLocalCount; cell_I++) {
    ...
    self->cellParticleTbl[cell_I] = Memory_Alloc_Array(
        Particle_Index, self->cellParticleSizeTbl[cell_I], tempStr);
  }
}
...
void _Swarm_BuildShadowParticles(void *swarm) {
  ...
  self->shadowCellParticleCountTbl =
      Memory_Alloc_Array(Particle_InCellIndex, self->cellDomainCount,
                         "Swarm->shadowCellParticleCountTbl");
  self->shadowCellParticleTbl = Memory_Alloc_Array(
      Cell_Particles, self->cellDomainCount, "Swarm->shadowCellParticleTbl");
...
void _Swarm_InitialiseCells(void *swarm, void *data) {
  ...
  for (cell_I = 0; cell_I < self->cellDomainCount; cell_I++) {
    CellLayout_InitialiseCellPoints(self->cellLayout, cell_I,
                                    self->cellPointCountTbl[cell_I],
...
void _Swarm_InitialiseParticles(void *swarm, void *data) {
  ...
  /* Need to re-do the delta calculation here, since if using a global particle
   * layouts, we didn't know the localParticleCount until the above call, and
   * had just used a default delta. We can now calculate the correct one. */
  self->particlesArrayDelta =
      (Particle_Index)((double)self->particleLocalCount *
                       self->extraParticlesFactor);
  if (0 == self->particlesArrayDelta) {
...
void Swarm_UpdateAllParticleOwners(void *swarm) {
...

  {
    ...
    for (ii = 0; ii < self->commHandlerList->count; ii++) {
      ...
      pComm =
          (ParticleCommHandler *)(Stg_ObjectList_At(self->commHandlerList, ii));
...
void Swarm_UpdateParticleOwner(void *swarm, Particle_Index particle_I) {
  ...
  GlobalParticle *particle =
      (GlobalParticle *)Swarm_ParticleAt(self, particle_I);
  ...
  if (newOwningCell != particle->owningCell) { /* if not still in same cell */
    if (particle->owningCell < self->cellDomainCount) { 
      /* if currently owned, remove from cell */
      Cell_LocalIndex oldOwningCell = particle->owningCell;
      cParticle_I = Swarm_GetParticleIndexWithinCell(self, particle->owningCell,
                                                     particle_I);
...
void Swarm_RemoveParticleFromCell(void *swarm, Cell_DomainIndex dCell_I,
                                  Particle_InCellIndex cParticle_I) {
  Swarm *self = (Swarm *)swarm;
  ...
  self->cellParticleTbl[dCell_I][cParticle_I] =
      self->cellParticleTbl[dCell_I][*countPtr - 1];
  (*countPtr)--;
  if (*countPtr == (*sizePtr - self->cellParticleTblDelta)) {
    (*sizePtr) = *countPtr;
    self->cellParticleTbl[dCell_I] = Memory_Realloc_Array(
        self->cellParticleTbl[dCell_I], Particle_Index, *sizePtr);
  }
}
...
void Swarm_DeleteParticle(void *swarm, Particle_Index particleToDelete_lI) {
  ...
  Journal_Firewall(particleToDelete_lI < self->particleLocalCount, errorStr,
                   "Error- in %s(): particleToDelete_lI passed in (%u) is >= "
                   "swarm's local particle count %u.\n",
                   __func__, particleToDelete_lI, self->particleLocalCount);

  particleToDelete =
      (GlobalParticle *)Swarm_ParticleAt(self, particleToDelete_lI);
  cParticle_I = Swarm_GetParticleIndexWithinCell(
      self, particleToDelete->owningCell, particleToDelete_lI);
  ...
  /* In the current data structure for particles (a regular array), if we delete
   * a particle we need to "swap" the 'last' particle into the hole we just
   * created, and update it's cell's reference to it. The only special case is
   * if the particle we are deleting happens to be the last particle, in which
   * case no swap is necessary. */
  if (particleToDelete_lI != lastParticle_I) {
    ...
    lastParticle_IndexWithinCell = Swarm_GetParticleIndexWithinCell(
        self, lastParticle_CellIndex, lastParticle_I);

    /* Copy over particle */
    memcpy(particleToDelete, lastParticle, particleSize);

    /* Change value in cell particle table to point to new index in array */
    self->cellParticleTbl[lastParticle_CellIndex]
                         [lastParticle_IndexWithinCell] = particleToDelete_lI;
  }

  /* re-set memory at location of last particle to zero so it is clear that it's
   * been deleted */
  memset(lastParticle, 0, particleSize);

  self->particleLocalCount--;
  /* Call the memory management function in case we need to re-allocate the
   * swarm size smaller now */
  Swarm_Realloc(swarm);
}

void Swarm_DeleteParticleAndReplaceWithNew(
    void *swarm, Particle_Index particleToDelete_lI, void *replacementParticle,
    Cell_Index replacementParticle_cellIndex) {
  Swarm *self = (Swarm *)swarm;
  Particle_InCellIndex cParticle_I = 0;
  GlobalParticle *particleToDelete = NULL;
  Stream *errorStr = Journal_Register(Error_Type, (Name)self->type);
  ...
  Journal_DPrintfL(self->debug, 2,
                   "Copying over particle %u using replacement particle, and "
                   "adding it to cell %u\n",
                   particleToDelete_lI, replacementParticle_cellIndex);

  /* Copy over particle to delete with it's replacement */
  memcpy(particleToDelete, replacementParticle,
         self->particleExtensionMgr->finalSize);

  /* Add a reference to replacement particle in appropriate cell entry */
  Swarm_AddParticleToCell(self, replacementParticle_cellIndex,
                          particleToDelete_lI);
}

void Swarm_AddParticleToCell(void *swarm, Cell_DomainIndex dCell_I,
  ...
  if (*newCountPtr == *newSizePtr) {
    ...
    self->cellParticleTbl[dCell_I] = Memory_Realloc_Array(
        self->cellParticleTbl[dCell_I], Particle_Index, (*newSizePtr));
  }
  self->cellParticleTbl[dCell_I][*newCountPtr] = particle_I;
  (*newCountPtr)++;
}

void Swarm_AddShadowParticleToShadowCell(void *swarm, Cell_DomainIndex dCell_I,
                                         Particle_Index shadowParticle_I) {
  Swarm *self = (Swarm *)swarm;

  if (self->shadowTablesBuilt) {
    Particle_InCellIndex *newCountPtr =
        &self->shadowCellParticleCountTbl[dCell_I - self->cellLocalCount];

    Journal_DPrintfL(self->debug, 3,
                     "Adding shadow particle %d to shadow cell %d: shadow "
                     "cell's particle count now %d",
...
Particle_InCellIndex
Swarm_GetParticleIndexWithinCell(void *swarm, Cell_DomainIndex owningCell,
                                 Particle_Index particle_I) {
  Swarm *self = (Swarm *)swarm;
  Particle_InCellIndex cParticle_I;
  ...
  for (cParticle_I = 0; cParticle_I < particleCount; cParticle_I++) {
    if (particle_I == self->cellParticleTbl[owningCell][cParticle_I])
      break;
  }
#if DEBUG
  Journal_Firewall(cParticle_I < self->cellParticleCountTbl[owningCell],
                   Swarm_Error,
                   "Error- In func %s: Particle %u not found among cell %u's "
                   "%u particles.\n",
...
Particle_Index Swarm_FindClosestParticle(void *_swarm, Dimension_Index dim,
                                         double *coord, double *distance) {
  ...
  cParticle_I =
      Swarm_FindClosestParticleInCell(swarm, lCell_I, dim, coord, &minDistance);
  ...
  Mesh_GetIncidence(((ElementCellLayout *)swarm->cellLayout)->mesh, dim,
                    lCell_I, dim, swarm->incArray);
  ...
  for (neighbour_I = 0; neighbour_I < neighbourCount; neighbour_I++) {
    ...
    if (lCell_I < swarm->cellDomainCount) {
      cParticle_I = Swarm_FindClosestParticleInCell(swarm, lCell_I, dim, coord,
                                                    &distanceToParticle);
...
Particle_InCellIndex Swarm_FindClosestParticleInCell(void *swarm,
  ...
  /* TODO: need to reconsider - gauss particle layout should be allowed, but not
   * swarms that have no local co-ordinates */
  /*
  Journal_Firewall(
  ...
  */
  ...
  Journal_Firewall(dCell_I < self->cellDomainCount, Swarm_Error,
                   "Bad Cell_DomainIndex %u.\n", dCell_I);

  /* Loop over particles find closest to vertex */
  for (cParticle_I = 0; cParticle_I < self->cellParticleCountTbl[dCell_I];
       cParticle_I++) {
    particle =
        (GlobalParticle *)Swarm_ParticleInCellAt(self, dCell_I, cParticle_I);

    /* Calculate distance to particle */
    distanceToParticle = (particle->coord[I_AXIS] - coord[I_AXIS]) *
                             (particle->coord[I_AXIS] - coord[I_AXIS]) +
                         (particle->coord[J_AXIS] - coord[J_AXIS]) *
                             (particle->coord[J_AXIS] - coord[J_AXIS]);

    if (dim == 3) {
      distanceToParticle += (particle->coord[K_AXIS] - coord[K_AXIS]) *
                            (particle->coord[K_AXIS] - coord[K_AXIS]);
    }
    /* Don't do square root here because it is unnessesary: i.e. a < b <=>
     * sqrt(a) < sqrt(b) */
...
void Swarm_PrintParticleCoords_ByCell(void *swarm, Stream *stream) {
  ...
  for (lCell_I = 0; lCell_I < self->cellLocalCount; lCell_I++) {
    ...
    for (cParticle_I = 0; cParticle_I < self->cellParticleCountTbl[lCell_I];
         cParticle_I++) {
      ...
      Journal_Printf(
          stream,
          "\tpart. InCell %3d (local index %6d): (%7.5g, %7.5g, %7.5g)\n",
...
void Swarm_GetCellMinMaxCoords(void *swarm, Cell_DomainIndex cell_I, Coord min,
                               Coord max) {
  Swarm *self = (Swarm *)swarm;
...
SwarmVariable *Swarm_NewScalarVariable(void *swarm, Name nameExt,
                                       int dataOffset,
  ...
  if (dataOffset == -1) {
    ...
    ExtensionManager_Add(self->particleExtensionMgr, (Name)self->type,
                         StgVariable_SizeOfDataType(dataType));
  }
  Journal_Firewall(
      dataOffset < ExtensionManager_GetFinalSize(self->particleExtensionMgr),
      Journal_MyStream(Error_Type, self),
      "Error in func %s - Failed to create Variable from extension: data "
      "offset within particle structure provided was invalid.\n"
      "\t%s %s\n"
      "\tdataOffset = %d, particle size = %d\n",
      __func__, self->type, self->name, dataOffset,
  ...
  variable = StgVariable_New(name, self->context, 1, dataOffsets, dataTypes,
                             dataTypeCounts, 0, /* no component names */
                             &self->particleExtensionMgr->finalSize,
                             &self->particleLocalCount, NULL,
                             (void **)&self->particles, variable_Register);

  swarmVariable =
      SwarmVariable_New(name, self->context, self, variable, 1, False);
...
SwarmVariable *Swarm_NewVectorVariable(void *_swarm, Name nameExt,
                                       int dataOffset,
                                       StgVariable_DataType dataType,
                                       Index dataTypeCount,
  ...
  if (dataOffset == -1) {
    dataOffset = ExtensionManager_GetFinalSize(self->particleExtensionMgr);
    ExtensionManager_Add(self->particleExtensionMgr, (Name)self->type,
                         dataTypeCount * StgVariable_SizeOfDataType(dataType));
  }
  Journal_Firewall(
      dataOffset < ExtensionManager_GetFinalSize(self->particleExtensionMgr),
      Journal_MyStream(Error_Type, self),
  ...
  if (!first) { // if NULL, continue add names programatically
    ...
    for (vector_I = 0; vector_I < dataTypeCount; vector_I++) {
      if (dataTypeCount > 1)
        sprintf((char *)suffix, "%u", vector_I);
    ...
    for (vector_I = 0; vector_I < dataTypeCount; vector_I++) {
      dataNames[vector_I] =
          Stg_Object_AppendSuffix(self, (Name)va_arg(ap, Name));
    }
    va_end(ap);
  }
  ...
  variable = StgVariable_New(
      name, self->context, 1, dataOffsets, dataTypes, dataTypeCounts, dataNames,
      &self->particleExtensionMgr->finalSize, &self->particleLocalCount, NULL,
      (void **)&self->particles, variable_Register);
  ...
  if (swarmVariable_Register && variable_Register) {
    for (vector_I = 0; vector_I < dataTypeCount; vector_I++) {
      swarmVariable = SwarmVariable_New(
          dataNames[vector_I], self->context, self,
          Variable_Register_GetByName(variable_Register, dataNames[vector_I]),
          1, False);
    }
    Memory_Free(dataNames[vector_I]);
  }
  swarmVariable = SwarmVariable_New(name, self->context, self, variable,
                                    dataTypeCount, False);
...
StgVariable *Swarm_GetShadowVariable(void *_swarm, StgVariable *variable) {
  ...
  return StgVariable_New(
      NULL, NULL, 1, variable->offsets, variable->dataTypes,
      variable->dataTypeCounts, NULL, &self->particleExtensionMgr->finalSize,
      &self->shadowParticleCount, NULL, (void **)&self->shadowParticles, NULL);
}
...
void Swarm_Realloc(void *swarm) {
  ...
  if (!self->expanding &&
      particleLocalCount <= self->particlesArraySize - delta) {
    ...
    reallocSwarm = True;
  } else if (self->particleExtensionMgr->finalSize !=
             self->preReallocParticleSize) {
  ...
  if (reallocSwarm)
    self->particles = Memory_Realloc_Array_Bytes(
        self->particles, self->particleExtensionMgr->finalSize,
        self->particlesArraySize);
  ...
  if (self->particleExtensionMgr->finalSize > self->preReallocParticleSize) {
    ...
    for (p_i = particleLocalCount - 1; p_i > 0; p_i--) {
      size_t oldLocation =
          (size_t)self->particles + p_i * self->preReallocParticleSize;
      size_t newLocation =
          (size_t)self->particles + p_i * self->particleExtensionMgr->finalSize;
    ...
    Journal_Firewall(
        NULL, NULL,
        "Error in func %s - Particle contractions not currently supported.",
        __func__);
    //        /* if particles contracting, work forward */
    //        for (p_i=1; p_i<particleLocalCount; p_i++) {
    //            size_t oldLocation = (size_t)self->particles +
    //            p_i*self->preReallocParticleSize; size_t newLocation =
    //            (size_t)self->particles +
    //            p_i*self->particleExtensionMgr->finalSize; memcpy(newLocation,
    //            oldLocation, self->preReallocParticleSize);
    //        }
  }
  ...
  if (reallocSwarm) {
    ...
    */
    ...
    if (self->particlesArraySize > particleLocalCount) {
      Particle_Index newParticleCount =
          self->particlesArraySize - particleLocalCount;
      void *startNewParticlePtr = Swarm_ParticleAt(self, particleLocalCount);
      memset(startNewParticlePtr, 0,
             self->particleExtensionMgr->finalSize * newParticleCount);
    }
#endif
  }
}
...
void Swarm_CheckCoordsAreFinite(void *swarm) {
  ...
  for (lParticle_I = 0; lParticle_I < particleLocalCount; lParticle_I++) {
    ...
    Journal_Firewall(
        ...
        errorStream,
        "Error in func %s - Coord for particle with %u is not finite (%g, %g, "
        "%g).\n",
        __func__, lParticle_I, coord[0], coord[1], (dim == 3 ? coord[2] : 0.0));
  }
}

void Swarm_AssignIndexWithinShape(void *swarm, void *_shape,
                                  StgVariable *variableToAssign,
                                  Index indexToAssign) {
...
StandardParticle *Swarm_CreateNewParticle(void *swarm,
...
Bool Swarm_AddCommHandler(Swarm *self, void *commHandler) {
  ...
  if (IsChild(((Stg_Component *)commHandler)->type, ParticleCommHandler_Type)) {
    ...
    Journal_Firewall(0, self->debug,
                     "Trying to add a class which is not of the type "
                     "ParticleCommHandler..!\n ");
    return False;
  }
}
...
void Swarm_AddVariable(Swarm *self, SwarmVariable *swarmVar) {
  ...
  self->swarmVars = MemRearray(self->swarmVars, SwarmVariable *,
                               self->nSwarmVars + 1, SwarmClass_Type);
