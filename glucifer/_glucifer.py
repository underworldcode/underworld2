...
class Store(_stgermain.StgCompoundComponent):
    ...
    def lvargs(self, args=[]):
        return [self._lvbin, self._db.path, "-" + str(self.step), "-p0", "-h", "-a"] + args
...
class Figure(dict):
    ...
    def _generate_image(self, filename="", size=(0,0)):
        ...
        try:
            #Render with viewer
            lv = LavaVu.load(self.db.lvargs(["-z" + str(self.quality)] + self._script))
    ...
    def _generate_HTML(self):
        ...
        try:
            ...
            lv = LavaVu.load(self.db.lvargs(self._script))
