...
while step < steps_end:
    ...
    if (step % steps_output == 0) or (step == steps_end-1):
        # These are not parallel aware commands !
        if uw.rank()==0:
            print 'step =',step, '; CPU time =', time.clock()-start, '; vrms ',Rms

            np.savetxt( outputPath + 'Summary.txt', np.c_[stepsVal, timeVal, viscdiscVal, nu0Val, nu1Val, Avg_tempVal, vrmsVal, Rms_surfVal, Max_vx_surfVal, GravworkVal, etamaxVal, etaminVal], header="Steps, Time, visc, Nusselt bottom/top, av_T, vrms, surf_rms, surf_max, grav, etamax, etamin" )
