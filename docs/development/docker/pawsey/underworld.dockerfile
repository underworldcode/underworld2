...
# RUN  pip3 install --no-cache-dir --no-build-isolation --no-binary=h5py h5py 
...
### fix for error: ImportError: /opt/cray/pe/gcc-libs/libstdc++.so.6: version `GLIBCXX_3.4.32' not found (required by /usr/local/lib/libpragmatic.so) ???
